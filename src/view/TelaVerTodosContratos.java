 ...
 * <161080391>
 */
public class TelaVerTodosContratos extends javax.swing.JFrame {
    ArrayList<Dados> todos = new ArrayList();
    /**
     * Creates new form TelaVerTodosContratos
     */
    public TelaVerTodosContratos(ArrayList<Dados> todo) {
        initComponents();
        this.todos = todo;
        preencherTabela();
    }

    private TelaVerTodosContratos() {
    ...
    private void btvercontratosActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btvercontratosActionPerformed
        int j = 0, v = -1;
        if (!todos.isEmpty()) {//Verifica se o array está vazio
            String x = campopesquisa.getText();//Se o array não estiver vazio, pega o cpf digitado
            if (!"".equals(campopesquisa.getText())) {
                for (int p = 0; p < todos.size(); p++) {//procura o cpf no array
                    Dados sa = todos.get(p);
                    if (x.equals(sa.getContrato().getCliente().getCpf())) {//Verifique se o cpf está no array
                        j = p;
                        v = p;
                        new TelaVerContratos(j, todos).setVisible(true);//mostra o contrato de acordo com o cpf
                        break;
                    }
                }
            }
